...
**Objective:** To present and evaluate a hybrid decentralized health data infrastructure that unifies authentication, informed consent, and session authorization into a single cryptographic event for behavioral health data governance.

**Design:** Proof-of-concept system architecture and prototype implementation, with comparative evaluation against four prior blockchain-based consent and health data systems.

**Methodology:** EIP-191 Ethereum wallet signatures are used as the unified primitive for identity verification, informed consent, and session authorization. The architecture follows a hybrid on-chain/off-chain model: consent attestations, achievement credentials, and engagement records are anchored to Ethereum Sepolia testnet; session metadata and encrypted documents are stored off-chain in PostgreSQL and IPFS. Server-side signature verification authorizes JWT issuance with HMAC-SHA256, delivered as HttpOnly cookies. A three-tier zero-PHI data classification model ensures no protected health information enters the system at any layer.

**Setting:** Testnet deployment on Ethereum Sepolia with five interconnected wellness and educational engagement platforms operating outside HIPAA regulatory scope by architectural design.

**Participants:** Single Sovereign Guide (principal investigator) for architectural validation. No external participants enrolled; no organic adoption measured. Formal user study with external participants requires IRB approval and is planned as future work.

**Interventions:** Nine smart contracts deployed on Ethereum Sepolia covering token issuance, session governance, soulbound credential attestation, voluntary engagement recording, biosensor consent scaffolding, and community treasury operations.

**Main outcome measures:** Architectural verification of consent-as-authentication unification; comparative properties analysis against MedRec, ADvoCATE, Welzel et al. (2025), and US Patent 12,235,984; deployment metrics including contract gas costs, authentication latency, and verification model characteristics.

**Results:** All nine contracts deployed and verified on Sepolia (block range 10,610,642–10,848,153). Wallet authentication latency observed at 4–6 seconds across Brave Wallet, MetaMask, and Coinbase Wallet. Comparative analysis indicates that, among systems surveyed, FSL is the first to unify authentication, informed consent, session authorization, and behavioral health data attribution into a single cryptographic signature event. The zero-PHI architectural model places the system outside HIPAA regulatory scope rather than within compliance frameworks.

**Conclusions:** Wallet-signed consent provides a verifiable, attributable, revocable, and portable alternative to click-through consent in behavioral health contexts. The architectural pattern is generalizable beyond the FSL context. Limitations include single-evaluator deployment, testnet-only contracts, centralized governance components, and the constraints of stateless JWT revocation. Named future work includes IRB-approved participant study, mainnet deployment under multisig governance, and bilateral consent verification.

---

## Plain Language Summary

Most online health and wellness apps ask you to click "I agree" to terms of service. This click is legally binding but provides no cryptographic proof of what you consented to, and the app controls your data afterward. This paper describes an alternative system that uses an Ethereum wallet — the kind people use for cryptocurrency — to give consent through a cryptographic signature instead of a checkbox.

The wallet signature does three things at once: it proves who you are, it shows what you consented to, and it lets you access the platform. Because the signature is cryptographic, anyone can verify it independently. Because the wallet belongs to you, the consent belongs to you.

The system was designed to hold zero protected health information. Instead of trying to protect sensitive data, the architecture is built so sensitive data never enters in the first place. This means the system operates outside U.S. health privacy regulation (HIPAA) by design rather than by compliance.

Nine smart contracts have been deployed on a public Ethereum test network for proof of concept. The system has not yet been tested with external users. A formal study and full production deployment are planned next steps.

The wider impact: this approach may help future health and wellness platforms give people real ownership of their consent and engagement records rather than asking them to trust a company's privacy policy.
...
This paper presents an alternative architecture. FSL is a decentralized health data infrastructure comprising five interconnected platforms that use Ethereum wallet signatures (EIP-191) as the sole mechanism for identity, consent, and access control. In this system:
...
2. **Session Recording.** During the session, engagement data is recorded off-chain. The current implementation stores no session content — no clinical notes, diagnostic codes, or treatment plans. The on-chain record contains only the guide address, participant address, content hash, and timestamp. Client-side encrypted session recording with participant-held decryption keys is an in-development architectural extension under a pending provisional patent; it is not part of the current deployment.
...
All owner-controlled contracts are currently deployed from a single deployer wallet. This centralization is acknowledged as a limitation. Migration to multisig governance is the named successor architecture for production deployment — distributing administrative authority across multiple signing keys with configurable quorum thresholds. The multisig migration path falls within the scope of a pending provisional patent.
...
5. **Bilateral wallet-signed mutual authentication with client-side encrypted session recording.** The current SovereignSession contract records single-party (Guide-initiated) attestation. A bilateral extension — in which both Guide and Participant submit EIP-191 signatures before session start, verified on-chain, with client-side AES-256-GCM encrypted video stored via IPFS under participant-held decryption keys — is an in-development architectural extension under a pending provisional patent. Among systems surveyed, no existing telehealth platform implements this combination. This capability is not part of the current deployment.
...
**Figure 4.** Sovereign Guide attestation lifecycle. Pre-session phase: Guide submits EIP-191 signature to initiate session; single-party consent state recorded on-chain via SovereignSession. Active session phase: off-chain engagement via WebRTC; no session content stored on-chain. Session-complete phase: either party invokes endSession; contract emits SessionEnded event with timestamp and duration. Future architectural extension under a pending provisional patent: bilateral wallet-signed mutual authentication and client-side AES-256-GCM encrypted session recording with participant-held decryption keys. Among systems surveyed, no existing telehealth platform implements this combination.

---

## Funding Statement

This work received no external funding. All research, infrastructure deployment, and patent filings were self-financed by the author. There were no sponsors or funders with any role in study design, data collection and analysis, decision to publish, or preparation of the manuscript.

## Authorship Contribution Statement

This manuscript has a single author. The author conceived the work, designed the system architecture, implemented the deployed smart contracts, drafted the manuscript, and approved the final version for submission. The author takes complete responsibility for all aspects of the work and the integrity of the data.

## Acknowledgments

AI tools (Anthropic Claude, Anthropic Claude Code) were used for drafting assistance, technical review, copy editing, and code review during manuscript preparation. Specific uses included: drafting iterative manuscript sections, reformatting references, reviewing technical accuracy of cryptographic descriptions, and copy editing for clarity. The AI tools did not conduct analysis, generate research results, or produce figures. No AI-generated images appear in this manuscript; all figures are hand-built architectural diagrams. All architectural decisions, factual claims, and final manuscript content are the responsibility of the author.

The author thanks the Ethereum Foundation for the EIP-191 and related Ethereum Improvement Proposals that underpin this work.

## Data Availability Statement

All smart contracts referenced in this manuscript are publicly verifiable on the Ethereum Sepolia testnet at eth-sepolia.blockscout.com. Contract addresses are listed in Table 1. Source code repositories will be linked in the published version. This manuscript has been pinned to IPFS for verifiable timestamping; the content identifier (CID) will be linked in the published version. No human subjects data was collected; no IRB-protected datasets are referenced. Underlying smart contract bytecode is publicly available on-chain and verifiable via Blockscout.
...
[3] 45 C.F.R. § 160.103 — Definitions: protected health information, covered entity, business associate. Washington (DC): U.S. Department of Health and Human Services.

[4] Allen C. The path to self-sovereign identity [Internet]. Life with Alacrity; 2016 [cited 2026 May 14]. Available from: http://www.lifewithalacrity.com/2016/04/the-path-to-self-soverereign-identity.html
...
[8] Kuo TT, Kim HE, Ohno-Machado L. Blockchain distributed ledger technologies for biomedical and health care applications. J Am Med Inform Assoc. 2017;24(6):1211-1220. DOI: https://doi.org/10.1093/jamia/ocx068

[9] Azaria A, Ekblaw A, Vieira T, Lippman A. MedRec: using blockchain for medical data access and permission management. In: Proceedings of the International Conference on Open and Big Data (OBD); 2016. p. 25-30.

[10] Rantos K, Drosatos G, Demertzis K, et al. ADvoCATE: a consent management platform for personal data processing in the IoT using blockchain technology. In: Proceedings of the International Conference on Security for Information Technology and Communications; 2019. p. 300-313.

[11] Dagher GG, Mohler J, Milner M, Marella A, Muraleedharan N. Ancile: privacy-preserving framework for access control and interoperability of electronic health records using blockchain technology. Sustain Cities Soc. 2018;39:283-297. DOI: https://doi.org/10.1016/j.scs.2018.02.014

[12] Esposito C, De Santis A, Tortora G, Chang H, Choo KKR. Blockchain: a panacea for healthcare cloud-based data security and privacy? IEEE Cloud Comput. 2018;5(1):31-37. DOI: https://doi.org/10.1109/MCC.2018.011791712

[13] Mayer AH, da Costa CA, Righi RR. Electronic health records in a blockchain: a systematic review. Health Informatics J. 2020;26(2):1273-1288. DOI: https://doi.org/10.1177/1460458219866350

[14] Zyskind G, Nathan O, Pentland A. Decentralizing privacy: using blockchain to protect personal data. In: Proceedings of the IEEE Security and Privacy Workshops; 2015. p. 180-184. DOI: https://doi.org/10.1109/SPW.2015.27

[15] Welzel C, Ostermann M, Smith HL, Minssen T, Kirsten T, Gilbert S. Enabling secure and self determined health data sharing and consent management. NPJ Digit Med. 2025;8:560. DOI: https://doi.org/10.1038/s41746-025-01945-z

[16] US Patent No. 12,235,984. Blockchain-based healthcare data consent management system. Washington (DC): United States Patent and Trademark Office.

[17] Ethereum Foundation. EIP-191: signed data standard [Internet]. Ethereum Improvement Proposals; 2018 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-191

[18] Ethereum Foundation. EIP-712: typed structured data hashing and signing [Internet]. Ethereum Improvement Proposals; 2017 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-712

[19] Ethereum Foundation. EIP-1271: standard signature validation method for contracts [Internet]. Ethereum Improvement Proposals; 2018 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-1271

[20] Weyl EG, Ohlhaver P, Buterin V. Decentralized society: finding Web3's soul. SSRN Electronic Journal. 2022. DOI: https://doi.org/10.2139/ssrn.4105763

[21] Ethereum Foundation. ERC-5192: minimal soulbound NFTs [Internet]. Ethereum Improvement Proposals; 2022 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-5192

[22] U.S. Food and Drug Administration. General wellness: policy for low risk devices (FDA-2014-N-1039) [Internet]. Guidance for Industry and Food and Drug Administration Staff; 2019 [cited 2026 May 14]. Available from: https://www.fda.gov/regulatory-information/search-fda-guidance-documents/general-wellness-policy-low-risk-devices

[23] Ethereum Foundation. EIP-6963: multi injected provider discovery [Internet]. Ethereum Improvement Proposals; 2023 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-6963

[24] National Institute of Standards and Technology. NIST special publication 800-63B: digital identity guidelines — authentication and lifecycle management. Washington (DC): U.S. Department of Commerce; 2017.
...
[27] 45 C.F.R. § 164.514 — Other requirements relating to uses and disclosures of protected health information: de-identification standard. Washington (DC): U.S. Department of Health and Human Services.

[28] 42 C.F.R. Part 2 — Confidentiality of substance use disorder patient records. Washington (DC): Substance Abuse and Mental Health Services Administration.

[29] Federal Trade Commission. Health breach notification rule, 16 C.F.R. Part 318. Washington (DC): Federal Trade Commission; 2009.

[30] Jung CG. The archetypes and the collective unconscious. In: Collected works of C.G. Jung, Vol. 9, Part 1. Princeton (NJ): Princeton University Press; 1959.

[31] Vogelsteller F, Buterin V. ERC-20: token standard [Internet]. Ethereum Improvement Proposals; 2015 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-20

[32] Entriken W, Shirley D, Evans J, Sachs N. ERC-721: non-fungible token standard [Internet]. Ethereum Improvement Proposals; 2018 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-721

[33] Radomski W, Cooke A, Castonguay P, et al. ERC-1155: multi token standard [Internet]. Ethereum Improvement Proposals; 2018 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-1155

[34] Ethereum Foundation. ERC-4337: account abstraction using alt mempool [Internet]. Ethereum Improvement Proposals; 2021 [cited 2026 May 14]. Available from: https://eips.ethereum.org/EIPS/eip-4337

[35] Sheffer Y, Hardt D, Jones M. JSON Web Token best current practices. IETF RFC 8725; 2020.

[36] Internal Revenue Service. IRS Notice 2014-21: virtual currency guidance [Internet]. Washington (DC): Internal Revenue Service; 2014 [cited 2026 May 14]. Available from: https://www.irs.gov/pub/irs-drop/n-14-21.pdf
